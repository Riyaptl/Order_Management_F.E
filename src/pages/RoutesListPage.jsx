...
const RoutesListPage = () => {
  ...
  const handleUpdate = async (newName, newRoutes) => {
    if (!newName.trim()) {
      toast.error("Route name cannot be empty");
      return;
    }
    if (!newRoutes.length) {
      toast.error("Please enter at least one area");
      return;
    }

    try {
      const res = await dispatch(updateArea({ id: selectedRouteId, name: newName, areas: newRoutes })).unwrap();
  ...
  const handleCreate = async ({ name, routes }) => {
    if (!name.trim()) {
      toast.error("Route name cannot be empty");
      return;
    }
